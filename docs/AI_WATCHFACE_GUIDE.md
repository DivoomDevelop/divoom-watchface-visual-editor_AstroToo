...
- Positions **`x`, `y`, `w`, `h`** are in **logical pixels**. This AstroToo editor uses **480×480** preview; keep elements inside that square canvas.
...
3. **Pivot / 12 o’clock**: rotation pivot is `(x + w/2, y + h/2)`. At **12:00** (hour/min tests: minute 0; second tests: second 0) rotation angle is **0°**. Draw the needle so that at **0°** it points **upward** toward −Y on the 480×480 canvas.
